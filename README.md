...
| 形态 | 条件 |
|---|---|
| **PWA** | **无需下载安装**——前端由电脑端网关自动托管，手机浏览器打开 `http://<电脑IP>:8443/` 即自动加载（前端代码在仓库 `public/` 目录，随网关分发，部署时勿删）。仅需 Android Chrome / iOS Safari 14+；「添加到主屏幕」只是创建桌面快捷方式，不是下载安装；「任务提醒」需授予通知权限（仅前台有效） |
...
1. 获取 APK：下载构建好的 `app-debug.apk`（或按下方「Android App（进阶）」章节自行构建）
