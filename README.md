...
在手机上实时监管电脑上的 **DeepSeek Harness**：任务进度实时监控、流式对话、模型/权限切换、任务完成/审批/出错提醒、可视化浏览**电脑全部磁盘**（可预览/下载/复制路径）、**手机文件直接上传到电脑**。支持 **PWA（浏览器）** 与 **Android App（常驻后台）** 两种形态。
...
| 文件浏览 | 浏览电脑**全部磁盘**（可选白名单）、文本预览、下载、**一键复制完整路径** |
| 文件上传 | **手机文件直接上传到电脑任意目录**（多选、进度提示） |
| 外观主题 | 深色/浅色两档 |
...
```
> 网关默认**文件范围 = 整个磁盘**（手机可浏览电脑全部盘符、预览/下载/复制路径、上传文件到任意目录）。想收窄范围，启动时可加参数 `-Root "C:\Users\Lenovo\Desktop"`（详见文末「安全说明」）。
...
| **文件** | 浏览电脑**全部磁盘**：进目录、文本预览、下载、复制路径、上传文件 |
...
- **浏览文件**：文件页默认显示电脑全部盘符 → 逐级进入目录；点文本文件直接预览
- **复制路径**：任意文件/目录右侧的 📋 按钮一键复制完整路径（如 `C:\Users\me\a.txt`，粘贴到任意应用）；顶栏「📋 路径」复制当前所在目录
- **下载文件**：点开文件预览后，浏览器菜单可另存；或长按文件选择下载（浏览器行为）
- **上传文件到电脑**：文件页顶栏「⬆ 上传」→ 选择手机上的文件（可多选）→ 自动上传到**当前所在目录**，逐个显示进度/成功/失败
- **回上级**：文件页顶部「根目录」回到盘符列表
...
| `GET /fs/list?path=` | 列目录；`path` 为空且未限制时返回**全部盘符** |
| `GET /fs/read?path=` | 读取文本文件内容（≤512KB） |
| `GET /fs/download?path=` | 下载文件到手机 |
| `POST /fs/upload?dir=&name=` | 上传文件到电脑指定目录（body 为原始字节，上限默认 1024MB） |

认证：`X-Gateway-Token` 请求头（SSE 用 `?token=`）。

## 安全说明

> ⚠️ **重要变化（v1.1）**：文件服务默认从「只读白名单」改为「**整个磁盘可读 + 可上传**」——这是为了满足"手机读取电脑所有本地文件 + 上传"的需求。**拿到令牌的人 = 能读你电脑上所有文件、能把文件写进任意目录**，请务必保管好令牌。

- 网关绑定 0.0.0.0，**局域网内任何人拿到令牌即可完全控制 DSH 并读写电脑文件**（令牌默认随机生成，仅显示在启动窗口）。
- 默认文件范围 = **整个磁盘**（全部盘符可浏览/预览/下载/复制路径，可上传到任意目录）。
- **想收窄文件范围**：启动时加 `-Root "C:\Users\Lenovo\Desktop"`（`start-gateway.ps1 -Root <目录>`），手机就只能访问该目录。
- **限制上传大小**：启动时加 `-MaxUploadMb 200`（默认 1024 MB/文件）。
- 上传文件名会做安全清洗（只取文件名、防路径穿越），只能上传到 `dir` 参数指定的目录。
...
- 文件上传走原始字节流（非 multipart），浏览器/App 内点击上传即可；单文件上限默认 1024MB（可 `-MaxUploadMb` 调整）
